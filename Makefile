...
CFLAGS = -fPIC -Wall -g -O2 -I$(INCDIR) -I$(SRCDIR) -I$(PAHO_SRC)/MQTTClient-C/src -I$(PAHO_SRC)/MQTTPacket/src -I$(SRCDIR)/github.com/gogo/protobuf/protobuf $(shell pkg-config --cflags 'libprotobuf-c >= 1.0.0')
LDFLAGS =
LDADD = -lpaho-embed-mqtt3c $(shell pkg-config --libs 'libprotobuf-c >= 1.0.0')
RM = rm -f
NAME = ttn-gateway-connector
TARGET_LIB = lib$(NAME).so

SRCS = $(SRCDIR)/network.c $(APIDIR)/api.pb-c.c $(APIDIR)/protocol/protocol.pb-c.c $(APIDIR)/protocol/lorawan/lorawan.pb-c.c $(APIDIR)/gateway/gateway.pb-c.c $(APIDIR)/router/router.pb-c.c

PROTOC = protoc-c --c_out=$(SRCDIR) --proto_path=$(GOPATH)/src $(GOPATH)/src
...
# TODO: Update dependencies of other platforms
...
$(BINDIR)/$(NAME)_test: $(BINDIR)/$(TARGET_LIB)
	$(CC) -I$(INCDIR) src/test.c -o $@ -L$(BINDIR) -l$(NAME)
